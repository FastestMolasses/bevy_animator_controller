...
fn main() {
    App::new()
        ...
        .add_systems(Update, update_transition_parameter.run_if(in_state(GameState::Playing)))
        .run();
}

fn update_transition_parameter(
    time: Res<Time>,
    mut query: Query<&mut AnimatorController>,
) {
    for mut controller in query.iter_mut() {
        // Toggle "is_running" every 3 seconds
        let is_running = (time.elapsed_secs() / 3.0) as i32 % 2 == 1;
        controller.parameters_mut().set_bool("is_running", is_running);
    }
}
...
fn setup_scene(
    ...
    if let Some(controller) = build_transition_controller(&player_animations, ozz_assets) {
...
fn build_transition_controller(
    ...
    animation_layer.add_state(
        "Run".to_string(),
        AnimationState::Simple(run_state),
    );

    // Add Transitions
    animation_layer.add_transition(
        "Idle".to_string(),
        Transition {
